...
In this project we explore various branches of machine learning, in an attempt to give an partial overview of the domain. The studied datasets are chosen in order to provide interesting problems, while at the same time not requiring a computing power that would be forbidding for general use hardware.

You are encouraged to explore and test various approaches, found both by researching litterature and proposing original, personal methods.
...
\begin{itemize}
	...
	\end{itemize}
    \item provide an \textbf{{evaluation}} or multiple evaluations of the obtained estimator(s), thanks to scorings of your choice. Be \textbf{quantitative} and precise about the results. When you mention the score of an estimator, \textbf{always} explicitely mention whether it is a train, test, validation, or cross-validation score (see the "scoring" chapter in the class).
...
\subsection{General ML knowledge}%
\label{sub:ml}
...
\subsection{Training and fine tuning for vocal removal}%
\label{sub:uvr}

Ultimate vocal remover (\url{https://ultimatevocalremover.com/}) allows to remove the vocals from an audio file. The program uses an adaptation pre trained audio source separation model, called demucs (\url{adefossez/demucs}). However, the dataset that were used in order to obtain this model is not disclosed by UVR. The UVR repo only contains the inference code.

Your objective is to train your own model and compare its performance to that of the native UVR model. You can explore various datasets and various model architectures, or fine-tune prexisting models. Produce audio examples in order to compare the different approaches.

\subsection{Compression of a model for Food 101 classification}%
\label{sub:food101}

The Food 101 dataset (\url{https://huggingface.co/datasets/ethz/food101}) is a classical classification dataset. Many models have already been trained in order to solve this problem. You objective is to find the smallest, lightest possible neural network model that keeps a score close to the networks that achieve optimal performance. Here, we are interested in compressing an already trained network, in order to \textbf{decrease the size in memory, number of parameters, and inference time}. To do so, many methods exist, such a quantization, distillation and other methods that you should explore. You are encouraged to explore research articles and use methods extracted from those. Study the tradeoff between the amount of compression and the final test score.

\subsection{Finding the best model on a tabular dataset}%
\label{sub:tabular}

Solve the problem of predicting the dominant tree species in a forest area using the available features. You may use any model you want and you are encouraged to compare many types of models, including simple benchmarks.

Among the various methods explored, you must explore at least one type of neural network and one ensemble learning method.
